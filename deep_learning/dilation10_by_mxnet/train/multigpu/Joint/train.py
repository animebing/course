...
def main():
    ...
    train_iter = FileIter(root_dir=args.data_dir, flist_name=args.train_list, up=args.up,
                         rgb_mean=tuple(args.mean), batch_size=args.train_batch)
    if args.test_list != "":
    ...
    _, arg_params, aux_params = mx.model.load_checkpoint(args.params[0], args.params_epoch[0])
    _, arg_tmp, aux_tmp = mx.model.load_checkpoint(args.params[1], args.params_epoch[1])
    arg_params.update(arg_tmp)
    aux_params.update(aux_tmp)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--params_epoch", nargs='*', type=int,
    ...
    parser.add_argument("--batch_interval", type=int, default=10,
                        help="batch interval for batch_end_callback")
