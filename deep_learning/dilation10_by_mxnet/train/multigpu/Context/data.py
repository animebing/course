...
class FileIter(DataIter):
    ...
    """
    def __init__(self, root_dir, flist_name, up, rgb_mean=(117, 117, 117),
                img_size=[1024, 2048], data_name="data",
        ...
        self.data = {}
        self.data = np.zeros((self.batch_size, 3, self.img_size[0]+2*self.margin, self.img_size[1]+2*self.margin))
        if up:
            self.label = np.zeros((self.batch_size, self.img_size[0]*self.img_size[1]))
        else:
            self.label = np.zeros((self.batch_size, self.img_size[0]//8*self.img_size[1]//8))
