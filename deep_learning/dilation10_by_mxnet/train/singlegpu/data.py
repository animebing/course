...
class FileIter(DataIter):
    ...
    def __init__(self, root_dir, flist_name, up,
        ...
        self.data = {}
        self.data[self.data_name] = np.zeros((self.batch_size, 3, self.crop_size,self.crop_size))
        if up:
            self.data[self.label_name] = np.zeros((self.batch_size, self.exact_crop, self.exact_crop))
        else:
            self.data[self.label_name] = np.zeros((self.batch_size, self.exact_crop//8, self.exact_crop//8))
    ...
    def _read(self):
        for i in xrange(self.batch_size):
            ...
            self.data[self.data_name][i][:], self.data[self.label_name][i][:]= self._read_img(data_img_name, label_img_name)
            self.cursor += 1
