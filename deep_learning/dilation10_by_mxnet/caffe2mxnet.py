...
def predict(dataset_name, input_path, output_path):
    ...
    #prefix = 'dilation10_cityscapes'
    #num_round = 1;
    net = caffe.Net(dataset.model_path, dataset.pretrained_path, caffe.TEST)
    #input_shape = net.blobs['data'].data.shape
    net_keys = net.params.keys()
    #print("net_keys: ", len(net_keys), net_keys)
    #cityscapes_sym = csym.get_symbol()
    sym, input_shape = proto2symbol(dataset.model_path)
    # input_shape is list here
    input_shape = tuple(input_shape)
    arg_names = sym.list_arguments()
    #print("arg_names: ", len(arg_names), arg_names)
    arg_shapes, _, _ = sym.infer_shape(data=input_shape)
    #print("arg_shapes:, ", arg_shapes)
    #print("arg_shape size: ", len(arg_shapes))
    arg_shape_dict = dict(zip(arg_names, arg_shapes))
    arg_params = {}
    for name in net_keys:
        #print(name)
        if name != 'ctx_upsample':
            if name == "fc-final":
                arg_params['fc_final'+'_weight'] = mx.nd.zeros(arg_shape_dict['fc_final'+'_weight'])
                arg_params['fc_final'+'_weight'][:] = net.params[name][0].data
                arg_params['fc_final'+'_bias'] = mx.nd.zeros(arg_shape_dict['fc_final'+'_bias'])
                arg_params['fc_final'+'_bias'][:] = net.params[name][1].data
            else:
                arg_params[name+'_weight'] = mx.nd.zeros(arg_shape_dict[name+'_weight'])
                arg_params[name+'_weight'][:] = net.params[name][0].data
                #print(net.params[name][0].data)
                #print(np.max(np.abs(arg_params[name+'_weight'].asnumpy() - net.params[name][0].data)))
                arg_params[name+'_bias'] = mx.nd.zeros(arg_shape_dict[name+'_bias'])
                arg_params[name+'_bias'][:] = net.params[name][1].data
                #print(np.max(np.abs(arg_params[name+'_bias'].asnumpy() - net.params[name][1].data)))


    model = mx.model.FeedForward(ctx=mx.gpu(0), symbol = sym, arg_params=arg_params, aux_params={})
    model.save(dataset_name, 1)
    """
    #model_keys = model.arg_params.keys()
    #print(len(model_keys), model_keys)
    #tmp_2 = model.arg_params['conv1_1_weight']
    #print(np.abs(net.params['conv1_1'][0].data-tmp_2.asnumpy()))
