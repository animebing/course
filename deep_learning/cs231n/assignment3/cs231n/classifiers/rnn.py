...
class CaptioningRNN(object):
  ...
  def sample(self, features, max_length=30):
    ...
    # a loop.                                                                 #
    ###########################################################################
    word_num = len(self.word_to_idx)
    prev_h = affine_forward(features, W_proj, b_proj)[0]
    caption_cur = self._start*np.ones((N, 1), dtype=np.int32)
    x = word_embedding_forward(caption_cur, W_embed)[0].reshape(N, -1)
    H = W_proj.shape[1]
    prev_c = np.zeros((N, H))
    for i in xrange(max_length):
        if self.cell_type == 'rnn':
            ...
            h, prev_c = lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b)[0:2]
        scores = affine_forward(h, W_vocab, b_vocab)[0]
        probs = np.exp(scores-np.max(scores, axis=1,keepdims=True))
        probs /= np.sum(probs, axis=1, keepdims=True)
    
        caption_cur = np.array([np.random.choice(range(word_num), p=probs[j, :].ravel()) for j in xrange(N)])
        #caption_cur = np.argmax(scores, axis=1)
        caption_cur = caption_cur[:, np.newaxis]
        captions[:, i:i+1] = caption_cur
        x = word_embedding_forward(caption_cur, W_embed)[0].reshape(N, -1)
        prev_h = h
