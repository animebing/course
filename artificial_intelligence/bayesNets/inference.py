...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        """
        print("all var: ", bayesNet.variablesSet())
        print("query var: ", set(queryVariables))
        print("evid var: ", set(evidenceDict.keys()))
        """
        # grab all factors where we know the evidence variables (to reduce the size of the tables)
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        # print(currentFactorsList)
        # join hidden variable first, then eliminate it
        for var in eliminationOrder:

            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, var)
            if len(joinedFactor.unconditionedVariables()) > 1:  # if = 1, then the elimination variable is the only one uncondition variable,
                joinedFactor = eliminate(joinedFactor, var)     # we can ignore the factor
                currentFactorsList.append(joinedFactor)
        # join all left variables: evidence and query variable
        fullJoint = joinFactors(currentFactorsList)
