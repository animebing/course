...
def constructBayesNet(gameState):
    ...
    for housePos in gameState.getPossibleHouses():
        ...
        tmp = []
        for obsPos in gameState.getHouseWalls(housePos):
            tmp.append(OBS_VAR_TEMPLATE % obsPos)
        print(tmp)
    """
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVars.append(OBS_VAR_TEMPLATE % obsPos)
    x_pos_edges = [(X_POS_VAR, FOOD_HOUSE_VAR), (X_POS_VAR, GHOST_HOUSE_VAR)]
    y_pos_edges = [(Y_POS_VAR, FOOD_HOUSE_VAR), (Y_POS_VAR, GHOST_HOUSE_VAR)]
    edges += x_pos_edges + y_pos_edges
...
def fillObsCPT(bayesNet, gameState):
    ...
    pos = [topLeftPos, topRightPos, bottomLeftPos, bottomRightPos]
    pos2obs = []
    obs_vars = []
    for housePos in pos:
        tmp = []
        for obsPos in gameState.getHouseWalls(housePos):
            obs_vars.append(OBS_VAR_TEMPLATE % obsPos)
            tmp.append(OBS_VAR_TEMPLATE % obsPos)
        pos2obs.append(tmp)
    obs_factor_dict = {}
    for obs in obs_vars:
        obs_factor_dict[obs] = bn.Factor([obs], [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], bayesNet.variableDomainsDict())
    ...
    for i in xrange(len(pos)):
        ...
        for j in xrange(len(pos)):
            ...
            for obs in food_obs:
                ...
                obs_assign = {obs:RED_OBS_VAL, FOOD_HOUSE_VAR:HOUSE_VALS[i], GHOST_HOUSE_VAR:HOUSE_VALS[j]}
                obs_factor_dict[obs].setProbability(obs_assign, PROB_FOOD_RED)
                obs_assign = {obs:BLUE_OBS_VAL, FOOD_HOUSE_VAR:HOUSE_VALS[i], GHOST_HOUSE_VAR:HOUSE_VALS[j]}

                obs_factor_dict[obs].setProbability(obs_assign, 1-PROB_FOOD_RED)
                obs_assign = {obs:NO_OBS_VAL, FOOD_HOUSE_VAR:HOUSE_VALS[i], GHOST_HOUSE_VAR:HOUSE_VALS[j]}
                obs_factor_dict[obs].setProbability(obs_assign, 0)

            if i != j:
                for obs in ghost_obs:

                    obs_assign = {obs:RED_OBS_VAL, FOOD_HOUSE_VAR:HOUSE_VALS[i], GHOST_HOUSE_VAR:HOUSE_VALS[j]}
                    obs_factor_dict[obs].setProbability(obs_assign, PROB_GHOST_RED)

                    obs_assign = {obs:BLUE_OBS_VAL, FOOD_HOUSE_VAR:HOUSE_VALS[i], GHOST_HOUSE_VAR:HOUSE_VALS[j]}
                    obs_factor_dict[obs].setProbability(obs_assign, 1-PROB_GHOST_RED)

                    obs_assign = {obs:NO_OBS_VAL, FOOD_HOUSE_VAR:HOUSE_VALS[i], GHOST_HOUSE_VAR:HOUSE_VALS[j]}
                    obs_factor_dict[obs].setProbability(obs_assign, 0)
            for t in xrange(len(pos)):
                if t!=i and t!=j:

                    for obs in pos2obs[t]:
                        ...
                        obs_assign = {obs:NO_OBS_VAL, FOOD_HOUSE_VAR:HOUSE_VALS[i], GHOST_HOUSE_VAR:HOUSE_VALS[j]}
                        obs_factor_dict[obs].setProbability(obs_assign, 1)


    for obs in obs_factor_dict:
        bayesNet.setCPT(obs, obs_factor_dict[obs])
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    for assign in all_assigns:
        ...
        if prob > max_prob:
            max_prob = prob
            best_pos = assign[FOOD_HOUSE_VAR]
